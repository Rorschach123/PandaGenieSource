<#
.SYNOPSIS
    Standalone NDK build script for the calculator module.
    Compiles libcalculator.so for arm64-v8a and armeabi-v7a.

.DESCRIPTION
    Uses CMake + NDK toolchain to cross-compile the calculator native library
    without depending on the AndroidProject Gradle build.

.PARAMETER NdkPath
    Path to Android NDK. Defaults to $env:ANDROID_NDK_HOME or auto-detected from SDK.
...
Write-Host "=== Building calculator native library ===" -ForegroundColor Cyan
Write-Host "NDK: $ndk"
...
Write-Host ""
Write-Host "=== calculator native build complete ===" -ForegroundColor Green
