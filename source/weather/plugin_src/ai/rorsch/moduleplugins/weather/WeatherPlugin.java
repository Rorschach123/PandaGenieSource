...
public class WeatherPlugin implements ModulePlugin {
    ...
    public String invoke(Context context, String action, String paramsJson) throws Exception {
        try {
            ...
            switch (action) {
                case "getCurrentWeather": return getCurrentWeather(params);
                case "getForecast":       return getForecast(params);
                case "openPage": {
    ...
    private String getCurrentWeather(JSONObject params) throws Exception {
        String city = params.optString("city", "").trim();
        if (city.isEmpty()) throw new IllegalArgumentException(isZh() ? "请提供城市名称" : "city is required");

        double[] coords = geocode(city);
        String cityName = coords.length > 2 ? "" : city;
    ...
    private String getForecast(JSONObject params) throws Exception {
        String city = params.optString("city", "").trim();
        if (city.isEmpty()) throw new IllegalArgumentException(isZh() ? "请提供城市名称" : "city is required");
